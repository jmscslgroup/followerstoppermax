...
// Generated 11-Jul-2021 14:28:57
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"followerstoppermax\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(followerstoppermax_M, (NULL));
        followerstoppermax_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(followerstoppermax_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(followerstoppermax_M)
                == (NULL)) || rtmGetStopRequested(followerstoppermax_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(followerstoppermax_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
